...
def APP_PORT = 8080 // Port di host target yang akan di-map ke port 80 kontainer
    ...
    stages {
        ...
        stage('3. Scan Image with Trivy') {
            steps {
                ...
                sh "sudo trivy image --exit-code 1 --severity HIGH,CRITICAL --ignore-unfixed ${IMAGE_NAME}"
            }
        }
